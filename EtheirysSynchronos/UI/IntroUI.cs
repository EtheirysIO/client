...
using EtheirysSynchronos.Localization;
...
namespace EtheirysSynchronos.UI
{
    internal class IntroUi : Window, IDisposable
    {
        ...
        public IntroUi(WindowSystem windowSystem, UiShared uiShared, Configuration pluginConfiguration,
            FileCacheManager fileCacheManager) : base("Etheirys Synchronos Setup")
        {
        ...
        public override void Draw()
        {
            if (!_pluginConfiguration.AcceptedAgreement && !_readFirstPage)
            {
                ...
                ImGui.TextUnformatted("Welcome to Etheirys Synchronos");
                if (_uiShared.UidFontBuilt) ImGui.PopFont();
                ImGui.Separator();
                UiShared.TextWrapped("Etheirys Synchronos is a plugin that will replicate your full current character state including all Penumbra mods to other paired Etheirys Synchronos users. " +
                         ...
                         || !Directory.Exists(_pluginConfiguration.CacheFolder)))
            {
                ...
                {
                    UiShared.TextWrapped("To not unnecessary download files already present on your computer, Etheirys Synchronos will have to scan your Penumbra mod directory. " +
                                         "Additionally, a local cache folder must be set where Etheirys Synchronos will download its local file cache to. " +
                    ...
                    UiShared.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.db of Etheirys Synchronos in the Plugin Configurations folder of Dalamud. " +
            ...
            else
            {
                ...
                if (_pluginConfiguration.ClientSecret.ContainsKey(_pluginConfiguration.ApiUri) && _uiShared.ShowClientSecret)
                {
                    ...
                    ImGui.Separator();
                    UiShared.ColorTextWrapped("You are now ready to go. Press Finish to finalize the settings and open the Etheirys Synchronos main UI.", ImGuiColors.ParsedGreen);
                    ImGui.Separator();
                ...
                else
                {
                    ...
                    UiShared.ColorTextWrapped("Do not ever, under any circumstances, share your secret key to anyone! Likewise do not share your Etheirys Synchronos plugin configuration to anyone!", ImGuiColors.DalamudYellow);
